...
struct RKMonth: View {
                    ...
                    HStack() {
                        ForEach(row, id:  \.self) { column in
                            HStack() {
                                ...
                                Spacer()
                            }.background(cellBackground(self.isBetweenStartAndEnd(date: column),self.isSpecialDate(date: column)))
                        }
                    }
                }
    ...
    func cellBackground(_ isBetweenStartAndEnd: Bool, _ isSpecialDate: Bool) -> Color{
        var color : Color = Color.white
        if isBetweenStartAndEnd {
            color =  Color.calendarDateRangeBackground
        }
        if isSpecialDate {
            color =   Color.white
        }
...
struct RKMonth_Previews : PreviewProvider {
    static var previews: some View {
        RKMonth(isPresented: .constant(false), monthShowing: .constant(1),rkManager: RKManager(calendar: Calendar.current, minimumDate: Date(), maximumDate: Date().addingTimeInterval(60*60*24*365), mode: 0), monthOffset: 0)
    }
}
#endif
